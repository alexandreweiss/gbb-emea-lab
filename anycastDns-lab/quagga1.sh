...
asn_quagga=65007
bgp_routerId=172.20.20.132
bgp_network1=10.10.10.10/32
anycast_IP=10.10.10.10
azure_IP=172.20.20.0/24
routeserver_IP1=172.20.20.4
routeserver_IP2=172.20.20.5
dnsserver_IP1=172.20.20.68
dnsserver_IP2=172.20.20.69
...
## Update IPTABLES to do SNAT/DNAT
# To DNS
# DNAT to DNS + SNAT from NVA
sudo iptables -t nat -A PREROUTING -d $bgp_network1 -j DNAT --to-destination $dnsserver_IP1
sudo iptables -t nat -A POSTROUTING ! -s $azure_IP -d $dnsserver_IP1 -j SNAT --to-source $bgp_routerId

sudo iptables -t nat -A PREROUTING -d $bgp_network1 -j DNAT --to-destination $dnsserver_IP2
sudo iptables -t nat -A POSTROUTING ! -s $azure_IP -d $dnsserver_IP2 -j SNAT --to-source $bgp_routerId

## From DNS (reply)
# SNAT DNS to anycast IP when going on-prem
sudo iptables -t nat -A POSTROUTING -s $dnsserver_IP1 ! -d $azure_IP -j SNAT --to-source $anycast_IP
sudo iptables -t nat -A POSTROUTING -s $dnsserver_IP2 ! -d $azure_IP -j SNAT --to-source $anycast_IP
